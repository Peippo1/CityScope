...
export function H3ActivityLayer({ cells, selectedH3Cell, onSelectH3Cell }: { cells: ActivityCell[]; selectedH3Cell?: string | null; onSelectH3Cell?: (h3Cell: string) => void }) {
  ...
  return <><ol className="activity-list" aria-label="Highest activity H3 cells">
    {visibleCells.map((cell, index) => <li key={cell.h3_cell}><button type="button" className={selectedH3Cell === cell.h3_cell ? "activity-row is-selected" : "activity-row"} aria-label={`Select Area ${index + 1} with ${cell.total_journeys.toLocaleString()} journeys`} aria-pressed={selectedH3Cell === cell.h3_cell} onClick={() => onSelectH3Cell?.(cell.h3_cell)} title={`H3 zone ${cell.h3_cell}`}><span className="rank">#{index + 1}</span><span className="activity-copy"><strong>H3 zone {cell.h3_cell.slice(-6)}</strong><small>{cell.origin_journeys} starts · {cell.destination_journeys} arrivals</small></span><span className="journey-value">{cell.total_journeys.toLocaleString()}<small>journeys</small></span></button></li>)}
