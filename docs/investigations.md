...
```text
question -> ADK Runner + root LlmAgent -> City Data MCP -> trusted H3 candidates
                           \-> Maps Grounding Lite MCP -> current places
                                -> deterministic counts/ranks -> answer + map
```

`POST /investigate` accepts a registry-backed city and bounded client-carried context. The FastAPI agent connects to `CITYSCOPE_CITY_DATA_MCP_URL` (default `http://localhost:8001/mcp/`) using the official MCP Python client. It never imports the analytics implementation directly. Historical investigations support London, New York City, Chicago, and Washington, DC; optional current station availability uses a separately deployed live-data MCP for NYC, Chicago, Washington, DC, and Paris.

The ADK Runner uses a single root agent powered by Gemini through `google-genai`, using stable `gemini-3.5-flash` unless overridden by `CITYSCOPE_GEMINI_MODEL`. The API loads the repository-root `.env.local` at startup. The agent returns a typed decision: call an approved City Data or live-data tool, call the internal `maps.search_places` boundary, answer, or reject the question as unsupported. Cross-city results are synthesized deterministically from the typed `compare_cities` response after Gemini selects the normalized metric; no second model call can alter the ranking. The orchestration loop validates tool arguments and returns dataset metadata, source-tagged evidence, map layers, current places, limitations, and a concise sanitized ADK trace. Optional Gemma 4 journey-character scores are accepted only after local schema and bounds validation.
